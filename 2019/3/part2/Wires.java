...
public class Wires
{
    public static void main(String [] args)
    {
        ...
        for(int i = 0; i < wire1Lines.size(); i++)
        {
            for(int j = 0; j < wire2Lines.size(); j++)
            {
                ...
                {
                    int totalSteps = seg1.steps + seg2.steps;
                    totalSteps += (seg1.s.x == intersection.x) ?
                                  Math.abs(intersection.y - seg1.s.y) :
                                  Math.abs(intersection.x - seg1.s.x);
                    totalSteps += (seg2.s.x == intersection.x) ?
                                   Math.abs(intersection.y - seg2.s.y) :
                                   Math.abs(intersection.x - seg2.s.x);
    ...
    public static ArrayList<Line> getLines(String wire)
    {
        ...
        for(String path : paths)
        {
            ...
            ans.add(new Line(at, stop, steps));
            at = stop;
            steps += amount;
        }
...
class Line
{
    ...
    Line(Point s, Point e, int steps)
    {
        this.s = s;
        this.e = e;
        this.steps = steps;
